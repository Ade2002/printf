...
{
    unsigned_char_t exp_bits_on = 0, exp_all_on;
    ...
    if (flt_info != NULL)
    {
        ...
        for (i = 0; *(flt_info->mantissa + i) != '\0'; i++)
        {
            ...
            lsb_on = *(flt_info->mantissa + i + 1) == '\0' && *(flt_info->mantissa + i) == '1' ? TRUE : lsb_on;
            if (*(flt_info->mantissa + i) == '1')
                mant_bits_on++;
        }
        if (exp_all_on)
        {
            if ((flt_info->sign == '0' && mant_bits_on == 0) || (flt_info->sign == '1' && mant_bits_on == 0))
            {
                return (str_copy("inf"));
            }
            else if ((flt_info->sign == '0' && mant_bits_on == 1 && lsb_on) || (flt_info->sign == '0' && mant_bits_on == 2 && lsb_on && msb_on) || (flt_info->sign == '0' && mant_bits_on == i))
            {
                return (str_copy("nan"));
            }
        }
